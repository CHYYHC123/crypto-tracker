    ...
    <div className={cn('flex flex-col gap-1', className)}>
      <label className={cn('inline-flex items-center gap-2 cursor-pointer select-none', disabled && 'cursor-not-allowed opacity-50', labelClassName)} onKeyDown={handleKeyDown} onFocus={() => setIsFocused(true)} onBlur={() => setIsFocused(false)} tabIndex={disabled ? -1 : 0}>
        <div className="relative inline-flex items-center">
          <input ref={ref} type="checkbox" data-slot="checkbox" checked={isChecked} onChange={handleChange} onFocus={() => setIsFocused(true)} onBlur={() => setIsFocused(false)} disabled={disabled} className="sr-only" {...props} />
          <motion.div
            className={cn(
              'relative w-4 h-4 rounded border-2 transition-colors',
              isChecked ? 'bg-gray-600 border-gray-600' : 'bg-gray-800 border-gray-600',
          ...
          >
            <AnimatePresence>
              {isChecked && (
                <motion.div initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0 }} transition={{ duration: 0.15 }} className="absolute inset-0 flex items-center justify-center">
